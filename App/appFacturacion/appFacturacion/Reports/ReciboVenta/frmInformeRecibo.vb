...
Public Class frmInformeRecibo
    ...
    Sub ListaDetalle(ByVal id As String)
        Try
            Using db As New CodeFirst
                Dim v = (From rec In db.VentasRecibos Join ser In db.SERIES On rec.IDSERIE Equals ser.IDSERIE Where rec.IDRECIBO = id Select rec.IDRECIBO, rec.ANULADO, SERIE = ser.NOMBRE, rec.CONSECUTIVO, rec.MONEDA).FirstOrDefault()
                If Not v Is Nothing Then
                    If v.ANULADO = "N" Then
                        expDetalle.TitleText = "MOSTRAR DETALLE DEL RECIBO DE VENTA '" & v.SERIE & " - " & v.CONSECUTIVO & "'"
                        dtDetalle.DataSource = (From ven In db.Ventas Join ser In db.SERIES On ven.IDSERIE Equals ser.IDSERIE Join det In db.VentasRecibosDetalles On ven.IDVENTA Equals det.IDVENTA Where det.IDRECIBO = id _
