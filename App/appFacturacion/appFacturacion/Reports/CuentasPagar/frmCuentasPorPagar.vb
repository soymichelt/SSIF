    ...
    Sub llenar(Optional ByVal codigo As String = "", Optional ByVal nombre As String = "")
        Try
            Using db As New CodeFirst
                Dim consulta = (From pro In db.Proveedores Where pro.FACTURADO_C <> 0 And pro.ACTIVO = "S" And pro.N_PROVEEDOR.Contains(codigo) And pro.NOMBRES.Contains(nombre) Select pro.N_PROVEEDOR, PROVEEDOR = pro.NOMBRES & " " & pro.APELLIDOS, pro.RAZONSOCIAL, pro.TELEFONO, pro.PLAZO, LIMITE = pro.LIMITECREDITO, FACTURADO = If(pro.MONEDA.Equals(Config.cordoba), pro.FACTURADO_C + (pro.FACTURADO_D * Config.exchangeRate), (pro.FACTURADO_C / Config.exchangeRate) + pro.FACTURADO_D), DISPONIBLE = pro.LIMITECREDITO - If(pro.MONEDA.Equals(Config.cordoba), pro.FACTURADO_C + (pro.FACTURADO_D * Config.exchangeRate), (pro.FACTURADO_C / Config.exchangeRate) + pro.FACTURADO_D))
