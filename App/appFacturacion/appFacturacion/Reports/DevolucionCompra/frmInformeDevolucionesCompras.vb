        ...
        Try
            Using db As New CodeFirst
                If dtRegistro.Visible Then
                    Dim consulta = (From dev In db.ComprasDevoluciones Join ser In db.Series On ser.IDSERIE Equals dev.IDSERIE Join cli In db.Clientes On cli.IDCLIENTE Equals dev.IDPROVEEDOR Join tra In db.Empleados On tra.IDEMPLEADO Equals dev.IDEMPLEADO Where dev.IDPROVEEDOR IsNot Nothing And dev.FECHADEVOLUCION >= Fecha1 And dev.FECHADEVOLUCION <= Fecha2 And ser.Bodega.IDBODEGA.Contains(BodegaP) And ser.IDSERIE.Contains(SerieP) And tra.N_TRABAJADOR.Contains(IdEmpleadoP) And (tra.NOMBRES & " " & tra.APELLIDOS).Contains(EmpleadoP) Select ANULADO = If(dev.ANULADO.Equals("S"), "Anulado", ""), dev.IDDEVOLUCION, SERIE = ser.NOMBRE, dev.CONSECUTIVO, dev.FECHADEVOLUCION, N_CLIENTE = If(dev.ANULADO.Equals("N"), cli.N_CLIENTE, ""), CLIENTE = If(dev.ANULADO.Equals("N"), cli.NOMBRES & " " & cli.APELLIDOS, ""), N_VENDEDOR = If(dev.ANULADO.Equals("N"), tra.N_TRABAJADOR, ""), VENDEDOR = If(dev.ANULADO.Equals("N"), tra.NOMBRES & " " & tra.APELLIDOS, ""), CONDICION = If(dev.ANULADO.Equals("N"), If(dev.CREDITO, "Crédito", "Contado"), ""), MÓNEDA = If(dev.ANULADO.Equals("N"), If(dev.MONEDA.Equals(Config.dolar), "Dólar", "Córdoba"), ""), TAZACAMBIO = If(dev.ANULADO.Equals("N"), dev.TAZACAMBIO, Nothing), DESCUENTO_DIN = If(dev.ANULADO.Equals("N"), If(dev.MONEDA.Equals(Config.cordoba), dev.DESCUENTO_DIN_C, dev.DESCUENTO_DIN_D), Nothing), SUBTOTAL = If(dev.ANULADO.Equals("N"), If(dev.MONEDA.Equals(Config.cordoba), dev.SUBTOTAL_C, dev.SUBTOTAL_D), Nothing), IVA_DIN = If(dev.ANULADO.Equals("N"), If(dev.MONEDA.Equals(Config.cordoba), dev.IVA_DIN_C, dev.IVA_DIN_D), Nothing), TOTAL = If(dev.ANULADO.Equals("N"), If(dev.MONEDA.Equals(Config.cordoba), dev.TOTAL_C, dev.TOTAL_D), Nothing), dev.N, dev.CREDITO, MON = dev.MONEDA) _
                                   .Union _
                                   (From dev In db.ComprasDevoluciones Join ser In db.Series On ser.IDSERIE Equals dev.IDSERIE Join tra In db.Empleados On tra.IDEMPLEADO Equals dev.IDEMPLEADO Where dev.IDPROVEEDOR Is Nothing And dev.FECHADEVOLUCION >= Fecha1 And dev.FECHADEVOLUCION <= Fecha2 And ser.Bodega.IDBODEGA.Contains(BodegaP) And ser.IDSERIE.Contains(SerieP) And tra.N_TRABAJADOR.Contains(IdEmpleadoP) And (tra.NOMBRES & " " & tra.APELLIDOS).Contains(EmpleadoP) Select ANULADO = If(dev.ANULADO.Equals("S"), "Anulado", ""), dev.IDDEVOLUCION, SERIE = ser.NOMBRE, dev.CONSECUTIVO, dev.FECHADEVOLUCION, N_CLIENTE = "", CLIENTE = If(dev.ANULADO.Equals("N"), dev.PROVEEDORCONTADO, ""), N_VENDEDOR = If(dev.ANULADO.Equals("N"), tra.N_TRABAJADOR, ""), VENDEDOR = If(dev.ANULADO.Equals("N"), tra.NOMBRES & " " & tra.APELLIDOS, ""), CONDICION = If(dev.ANULADO.Equals("N"), If(dev.CREDITO, "Crédito", "Contado"), ""), MÓNEDA = If(dev.ANULADO.Equals("N"), If(dev.MONEDA.Equals(Config.dolar), "Dólar", "Córdoba"), ""), TAZACAMBIO = If(dev.ANULADO.Equals("N"), dev.TAZACAMBIO, Nothing), DESCUENTO_DIN = If(dev.ANULADO.Equals("N"), If(dev.MONEDA.Equals(Config.cordoba), dev.DESCUENTO_DIN_C, dev.DESCUENTO_DIN_D), Nothing), SUBTOTAL = If(dev.ANULADO.Equals("N"), If(dev.MONEDA.Equals(Config.cordoba), dev.SUBTOTAL_C, dev.SUBTOTAL_D), Nothing), IVA_DIN = If(dev.ANULADO.Equals("N"), If(dev.MONEDA.Equals(Config.cordoba), dev.IVA_DIN_C, dev.IVA_DIN_D), Nothing), TOTAL = If(dev.ANULADO.Equals("N"), If(dev.MONEDA.Equals(Config.cordoba), dev.TOTAL_C, dev.TOTAL_D), Nothing), dev.N, dev.CREDITO, MON = dev.MONEDA)
                    ...
                    If dtRegistro.Columns.Count > 0 Then
                        ...
                        dtRegistro.Columns(6).HeaderText = "NOMBRES Y APELLIDOS DEL PROVEEDOR" : dtRegistro.Columns(6).Width = 250
                    ...
                    End If
                Else
                    Dim consulta = (From dev In db.ComprasDevoluciones Join ser In db.Series On ser.IDSERIE Equals dev.IDSERIE Join cli In db.Clientes On cli.IDCLIENTE Equals dev.IDPROVEEDOR Join tra In db.Empleados On tra.IDEMPLEADO Equals dev.IDEMPLEADO Where dev.ANULADO.Equals(Config.vFalse) And dev.IDPROVEEDOR IsNot Nothing And dev.FECHADEVOLUCION >= Fecha1 And dev.FECHADEVOLUCION <= Fecha2 And ser.Bodega.IDBODEGA.Contains(BodegaP) And ser.IDSERIE.Contains(SerieP) And tra.N_TRABAJADOR.Contains(IdEmpleadoP) And (tra.NOMBRES & " " & tra.APELLIDOS).Contains(EmpleadoP) Select ser.IDBODEGA, BODEGA = ser.Bodega.N_BODEGA & " - " & ser.Bodega.DESCRIPCION, dev.IDDEVOLUCION, SERIE = ser.NOMBRE, dev.CONSECUTIVO, Fecha = dev.FECHADEVOLUCION, cli.N_CLIENTE, CLIENTE = cli.NOMBRES & " " & cli.APELLIDOS, N_VENDEDOR = tra.N_TRABAJADOR, VENDEDOR = tra.NOMBRES & " " & tra.APELLIDOS, CONDICION = If(dev.CREDITO, "Crédito", "Contado"), MONEDA = If(dev.MONEDA.Equals(Config.dolar), "Dólar", "Córdoba"), dev.TAZACAMBIO, DESCUENTO = If(dev.MONEDA.Equals(Config.cordoba), dev.DESCUENTO_DIN_C, dev.DESCUENTO_DIN_D), SUBTOTAL = If(dev.MONEDA.Equals(Config.cordoba), dev.SUBTOTAL_C, dev.SUBTOTAL_D), IVA = If(dev.MONEDA.Equals(Config.cordoba), dev.IVA_DIN_C, dev.IVA_DIN_D), TOTAL = If(dev.MONEDA.Equals(Config.cordoba), dev.TOTAL_C, dev.TOTAL_D), dev.N, dev.CREDITO, MON = dev.MONEDA) _
                                   .Union _
                                   (From dev In db.ComprasDevoluciones Join ser In db.Series On ser.IDSERIE Equals dev.IDSERIE Join tra In db.Empleados On tra.IDEMPLEADO Equals dev.IDEMPLEADO Where dev.ANULADO.Equals(Config.vFalse) And dev.IDPROVEEDOR Is Nothing And dev.FECHADEVOLUCION >= Fecha1 And dev.FECHADEVOLUCION <= Fecha2 And ser.Bodega.IDBODEGA.Contains(BodegaP) And ser.IDSERIE.Contains(SerieP) And tra.N_TRABAJADOR.Contains(IdEmpleadoP) And (tra.NOMBRES & " " & tra.APELLIDOS).Contains(EmpleadoP) Select ser.IDBODEGA, BODEGA = ser.Bodega.N_BODEGA & " - " & ser.Bodega.DESCRIPCION, dev.IDDEVOLUCION, SERIE = ser.NOMBRE, dev.CONSECUTIVO, Fecha = dev.FECHADEVOLUCION, N_CLIENTE = "", CLIENTE = dev.PROVEEDORCONTADO, N_VENDEDOR = tra.N_TRABAJADOR, VENDEDOR = tra.NOMBRES & " " & tra.APELLIDOS, CONDICION = If(dev.CREDITO, "Crédito", "Contado"), MONEDA = If(dev.MONEDA.Equals(Config.dolar), "Dólar", "Córdoba"), dev.TAZACAMBIO, DESCUENTO = If(dev.MONEDA.Equals(Config.cordoba), dev.DESCUENTO_DIN_C, dev.DESCUENTO_DIN_D), SUBTOTAL = If(dev.MONEDA.Equals(Config.cordoba), dev.SUBTOTAL_C, dev.SUBTOTAL_D), IVA = If(dev.MONEDA.Equals(Config.cordoba), dev.IVA_DIN_C, dev.IVA_DIN_D), TOTAL = If(dev.MONEDA.Equals(Config.cordoba), dev.TOTAL_C, dev.TOTAL_D), dev.N, dev.CREDITO, MON = dev.MONEDA)
