                    ...
                    Using db As New CodeFirst
                        ...
                        If Not v Is Nothing Then
                            ...
                            If Config.ValidateLapse(v.FECHAFACTURA) Then
                                ...
                                For Each kardexItemForThisSale In db.Kardexs.Where(Function(f) f.IDSERIE = v.IDSERIE And f.N_DOCUMENTO = txtCodigo.Text).ToList()

                                    'acumuladores de costos
                                    Dim accumulatedCostToDebit = 0, accumulatedCostToAcredit As Decimal = 0

                                    'item actual
                                    Dim kardexSelectedForThisSale = db.Kardexs.Where(Function(f) f.IDKARDEX = kardexItemForThisSale.IDKARDEX).Include(Function(f) f.Existencia).First()

                                    Dim postKardexs = db.Kardexs.Where(Function(f) f.N > kardexSelectedForThisSale.N And f.IDEXISTENCIA = kardexSelectedForThisSale.IDEXISTENCIA).OrderBy(Function(f) f.N).ToList()
                                    ...
                                    For Each postItemKardex In postKardexs
                                        ...
                                        currentPostItemKardex.EXISTENCIA_ANTERIOR = currentPostItemKardex.EXISTENCIA_ANTERIOR + kardexSelectedForThisSale.SALIDA
                                        currentPostItemKardex.EXISTENCIA_ALMACEN = currentPostItemKardex.EXISTENCIA_ALMACEN + kardexSelectedForThisSale.SALIDA
                                    ...
                                    Dim productForKardexItem = kardexSelectedForThisSale.Existencia.Producto

                                    productForKardexItem.SALDO += (accumulatedCostToDebit - accumulatedCostToAcredit)

                                    kardexSelectedForThisSale.ACTIVO = "N" : db.Entry(kardexSelectedForThisSale).State = EntityState.Modified
                                ...
                                For Each ventaDetalle In v.VentasDetalles.ToList()

                                    Dim item = db.VentasDetalles.Find(ventaDetalle.IDDETALLEVENTA)
                                    ...
                                    If item.Existencia.CANTIDAD < 0 Then
                                        If Not item.Existencia.Producto.FACTURAR_NEGATIVO Then
                                            Config.MsgErr("No se puede anular esta Venta. Ya que la existencia del producto '" & item.Existencia.Producto.IDALTERNO & " - " & item.Existencia.Producto.DESCRIPCION & "' quedaría en negativo.")
                                            Exit Sub
                                        End If
                                    End If
                ...
                If Not v Is Nothing Then
                    If v.ANULADO = "N" Then
                        If v.Serie.TICKET.Equals(Config.vTrue) Then
                            ...
                            rpt.SetDataSource((From ven In db.Ventas Join tra In db.Empleados On tra.IDEMPLEADO Equals ven.IDEMPLEADO Join ser In db.Series On ven.IDSERIE Equals ser.IDSERIE Join det In db.VentasDetalles On ven.IDVENTA Equals det.IDVENTA Join exi In db.Existencias On exi.IDEXISTENCIA Equals det.IDEXISTENCIA Join pro In db.Productos On exi.IDPRODUCTO Equals pro.IDPRODUCTO Where ven.IDVENTA = Me.Id And ven.ANULADO = "N" Select IDFACTURA = ven.IDVENTA, BODEGA = ser.Bodega.N_BODEGA & " | " & ser.Bodega.DESCRIPCION, SERIE = ser.NOMBRE, N_VENDEDOR = tra.N_TRABAJADOR, VENDEDOR = tra.NOMBRES & " " & tra.APELLIDOS, N_CLIENTE = If(Not ven.IDCLIENTE Is Nothing, ven.Cliente.N_CLIENTE, ""), CLIENTE = If(Not ven.IDCLIENTE Is Nothing, ven.Cliente.NOMBRES & " " & ven.Cliente.APELLIDOS, ven.CLIENTECONTADO), N_FACTURA = ven.CONSECUTIVO, FECHA = ven.FECHAFACTURA, CONDICION = If(ven.CREDITO, "Crédito", "Contado"), MARCA = pro.Marca.DESCRIPCION, ALTERNO = pro.IDALTERNO, ORIGINAL = pro.IDORIGINAL, pro.DESCRIPCION, det.CANTIDAD, PRECIO = If(ven.MONEDA.Equals(Config.cordoba), det.PRECIOUNITARIO_C, det.PRECIOUNITARIO_D), SUBTOTAL_DETALLE = If(ven.MONEDA.Equals(Config.cordoba), det.TOTAL_C, det.TOTAL_D), SUBTOTAL_FACTURA = If(ven.MONEDA.Equals(Config.cordoba), ven.SUBTOTAL_C, ven.SUBTOTAL_D), DESCUENTO = If(ven.MONEDA.Equals(Config.cordoba), ven.DESCUENTO_DIN_C, ven.DESCUENTO_DIN_D), IVA = If(ven.MONEDA.Equals(Config.cordoba), ven.IVA_DIN_C, ven.IVA_DIN_D), TOTAL = If(ven.MONEDA.Equals(Config.cordoba), ven.TOTAL_C, ven.TOTAL_D)).ToList())
