...
Public Class frmConsignacion
    ...
    Private Sub frmConsignacion_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            dtpFecha.Value = DateTime.Now
            Using db As New CodeFirst
                cmbSerie.DataSource = db.Series.Where(Function(f) f.IDBODEGA = Config.warehouseId And f.ACTIVO = "S" And f.OPERACION = "CONSIGNACION").ToList() : cmbSerie.ValueMember = "IDSERIE" : cmbSerie.DisplayMember = "NOMBRE" : cmbSerie.SelectedIndex = -1 : txtCodigo.Clear() : cmbSerie.Focus()
                txtReferencia.Clear()
            End Using
        Catch ex As Exception
            MessageBox.Show("Error, " & ex.Message)
