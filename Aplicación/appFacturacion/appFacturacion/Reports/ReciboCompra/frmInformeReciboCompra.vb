Public Class frmInformeReciboCompra
    ...
    Sub ListaDetalle(ByVal id As String)
        Try
            Using db As New CodeFirst
                Dim v = (From rec In db.COMPRAS_RECIBOS Join ser In db.SERIES On rec.IDSERIE Equals ser.IDSERIE Where rec.IDRECIBO = id Select rec.IDRECIBO, rec.ANULADO, SERIE = ser.NOMBRE, rec.CONSECUTIVO, rec.MONEDA).FirstOrDefault()
                If Not v Is Nothing Then
                    If v.ANULADO = "N" Then
                        expDetalle.TitleText = "MOSTRAR DETALLE DEL RECIBO DE COMPRA '" & v.SERIE & " - " & v.CONSECUTIVO & "'"
                        dtDetalle.DataSource = (From com In db.COMPRAS Join ser In db.SERIES On com.IDSERIE Equals ser.IDSERIE Join det In db.COMPRAS_RECIBOS_DETALLES On com.IDCOMPRA Equals det.IDCOMPRA Where det.IDRECIBO = id _
