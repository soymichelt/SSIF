    ...
    Sub Lista(ByVal Fecha1 As DateTime, ByVal Fecha2 As DateTime, Optional ByVal IDBodega As String = "", Optional ByVal IDSerie As String = "", Optional ByVal NEmpleado As String = "", Optional ByVal Empleado As String = "", Optional ByVal NProveedor As String = "", Optional ByVal Proveedor As String = "")
        Try
            Using db As New CodeFirst
                ...
                Dim SpSQL = db.Database.SqlQuery(Of lstProductosVendidos)("SpProductosComprados @Inicio, @Final, @IDBodega, @IDSerie, @NEmpleado, @Empleado, @NProveedor, @Proveedor, @TipoVenta, @MonInv, @Taza", New SqlParameter("@Inicio", Fecha1), New SqlParameter("@Final", Fecha2), New SqlParameter("@IDBodega", IDBodega), New SqlParameter("@IDSerie", IDSerie), New SqlParameter("@NEmpleado", NEmpleado), New SqlParameter("@Empleado", Empleado), New SqlParameter("@NProveedor", NProveedor), New SqlParameter("@Proveedor", Proveedor), New SqlParameter("@TipoVenta", If(rdContado.Checked, 1, If(rdCredito.Checked, 2, 0))), New SqlParameter("@MonInv", If(Config.Empresa.MonedaInventario.Equals(Config.cordoba), 1, 2)), New SqlParameter("@Taza", Config.tazadecambio)).ToList()
                ...
                If dtRegistro.Visible Then
                    dtRegistro.DataSource = (From c In SpSQL Select c.IDAlterno, c.Descripcion, c.Cantidad, c.PrecioPromedio, c.Descuento, c.SubTotal, c.Iva, c.Total Order By IDAlterno Descending).ToList
