Public Class frmInformeRecibo
            ...
            Using db As New CodeFirst
                If dtRegistro.Visible Then
                    ...
                    If IdClienteP.Trim <> "" Then 'filtro cliente
                        consulta = consulta.Where(Function(f) f.ncliente.Contains(IdClienteP) And f.ANULADO.Equals(""))
                    End If
                    ...
                    If dtRegistro.Columns.Count > 0 Then
                        ...
                        dtRegistro.Columns(5).Width = 150 : dtRegistro.Columns(5).HeaderText = "N° CLIENTE"
                        dtRegistro.Columns(6).Width = 250 : dtRegistro.Columns(6).HeaderText = "NOMBRES Y APELLIDOS DEL CLIENTE"
                        dtRegistro.Columns(7).Width = 120 : dtRegistro.Columns(7).HeaderText = "Nº EMPLEADO"
                        dtRegistro.Columns(8).Width = 250 : dtRegistro.Columns(8).HeaderText = "NOMBRES Y APELLIDOS DEL EMPLEADO"
                        dtRegistro.Columns(9).Width = 350 : dtRegistro.Columns(9).HeaderText = "CONCEPTO"
                ...
                Else
                    ...
                    End If
                    If IdClienteP.Trim <> "" Then 'filtro cliente
                        consulta = consulta.Where(Function(f) f.NProveedor.Contains(IdClienteP) And f.ANULADO.Equals(""))
                    End If
                    If ClienteP.Trim <> "" Then
                        consulta = consulta.Where(Function(f) f.Proveedor.Contains(ClienteP) And f.ANULADO.Equals(""))
                    End If
                    If IdEmpleadoP.Trim <> "" Then 'filtro vendedor
                        consulta = consulta.Where(Function(f) f.NEmpleado.Contains(IdEmpleadoP) And f.ANULADO.Equals(""))
                    End If
                    ...
                    Config.CrystalTitle("INFORME DE RECIBOS DE VENTA", rpt)
    ...
    Sub ListaDetalle(ByVal id As String)
        Try
            Using db As New CodeFirst
                Dim v = (From rec In db.RECIBOS Join ser In db.SERIES On rec.IDSERIE Equals ser.IDSERIE Where rec.IDRECIBO = id Select rec.IDRECIBO, rec.ANULADO, SERIE = ser.NOMBRE, rec.CONSECUTIVO, rec.MONEDA).FirstOrDefault()
                If Not v Is Nothing Then
                    If v.ANULADO = "N" Then
                        expDetalle.TitleText = "MOSTRAR DETALLE DEL RECIBO DE VENTA '" & v.SERIE & " - " & v.CONSECUTIVO & "'"
                        dtDetalle.DataSource = (From ven In db.VENTAS Join ser In db.SERIES On ven.IDSERIE Equals ser.IDSERIE Join det In db.DETALLES_RECIBOS On ven.IDVENTA Equals det.IDVENTA Where det.IDRECIBO = id _
                                   Select ven.IDVENTA, SERIE = ser.NOMBRE, ven.CONSECUTIVO, FECHA = ven.FECHAFACTURA, MÓNEDA = If(ven.MONEDA.Equals(Config.cordoba), "Córdoba", "Dólar"), ven.TAZACAMBIO, DESCUENTO = If(ven.MONEDA.Equals(Config.cordoba), ven.DESCUENTO_DIN_C, ven.DESCUENTO_DIN_D), SUBTOTAL = If(ven.MONEDA.Equals(Config.cordoba), ven.SUBTOTAL_C, ven.SUBTOTAL_D), IVA = If(ven.MONEDA.Equals(Config.cordoba), ven.IVA_DIN_C, ven.IVA_DIN_D), TOTAL = If(ven.MONEDA.Equals(Config.cordoba), ven.TOTAL_C, ven.TOTAL_D), det.SALDOCREDITO, OPERACION = If(det.OPERACION.Equals("A"), "ABONO", "CANCELACIÓN"), IMPORTE = If(v.MONEDA.Equals(Config.cordoba), det.IMPORTE_C, det.IMPORTE_D), DESCUENTO_REC = If(v.MONEDA.Equals(Config.cordoba), det.DESCUENTO_C, det.DESCUENTO_D), NUEVO_SALDO = If(v.MONEDA.Equals(Config.cordoba), det.NUEVO_SALDO_C, det.NUEVO_SALDO_D)).ToList
                        If dtDetalle.Columns.Count > 0 Then
                            ...
                            dtDetalle.Columns(2).HeaderText = "N° VENTA" : dtDetalle.Columns(2).Width = 100
                    ...
                    Else
                        dtDetalle.DataSource = Nothing
                        expDetalle.TitleText = "NO SE PUEDE MOSTRAR DETALLE DEL RECIBO DE VENTA '" & v.SERIE & " - " & v.CONSECUTIVO & "' POR QUE ESTÁ ANULADO."
                    End If
                Else
                    dtDetalle.DataSource = Nothing
                    MessageBox.Show("No se encuentra este Recibo de Venta")
                End If
                v = Nothing
            End Using
        Catch ex As Exception
    ...
    Private Sub frmInformeRecibo_Resize(sender As Object, e As EventArgs) Handles MyBase.Resize
        'If Me.LoadForm Then
    ...
    Private Sub frmInformeRecibo_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        frmInformeRecibo_Resize(Nothing, Nothing)
        Me.FormLoad = False
        'Formatos de Totales
