Public Class frmNotaDevolucionCompra
    ...
    Function generarCodigo(ByVal serie As String) As String
        Try
            Using db As New CodeFirst
                Dim compra = ((From com In db.COMPRAS Where com.IDSERIE = serie Select com.IDSERIE, com.CONSECUTIVO).Union(From dev In db.COMPRAS_DEVOLUCIONES Where dev.IDSERIE = serie Select dev.IDSERIE, dev.CONSECUTIVO)).OrderBy(Function(f) f.CONSECUTIVO).ToList.LastOrDefault
                            ...
                            If Not producto Is Nothing Then
                                ...
                                ElseIf Not txtCantidad.Text.Trim = "" And Not txtPrecio.Text.Trim = "" Then
                                    ...
                                    If rdCredito.Checked Then
                                        ...
                                        If Not Compra Is Nothing Then
                                            If Compra.DETALLES_COMPRAS.Where(Function(f) f.EXISTENCIA.PRODUCTO.IDALTERNO = txtCodigoAlterno.Text).FirstOrDefault Is Nothing Then
                ...
                If txtIdSerie.Text <> "" And txtNDevolucion.Text.Trim <> "" And txtObservacion.Text.Trim <> "" And Not dtRegistro.Rows.Count = 0 Then
                    Using db As New CodeFirst
                        ...
                        If Not txtCodigo.Text.Trim() = "" Then
                            ...
                            Dim v As New COMPRA_DEVOLUCION
                            v.Reg = DateTime.Now
                            ...
                            Else
                                    ...
                                    If Not c Is Nothing Then
                                        ...
                                        If Not Compra Is Nothing Then
                                            If Compra.SALDOCREDITO >= If(Compra.MONEDA.Equals(cordoba), If(rdCordoba.Checked, txtTotal.Value, txtTotal.Value * txtTazaCambio.Value), If(rdCordoba.Checked, txtTotal.Value / txtTazaCambio.Value, txtTotal.Value)) Then
                                                ...
                                                Dim est As New COMPRA_PAGO_PROVEEDOR
                                                ...
                                                db.COMPRAS_PAGOS_PROVEEDORES.Add(est)
                                                est = Nothing
                                ...
                                End If
                            End If
                            Dim producto As EXISTENCIA : Dim cont As Integer = 0 : Dim d As COMPRA_DEVOLUCION_DETALLE
                            For Each item In detalles
                                ...
                                If Not producto Is Nothing Then
                                    ...
                                    If Compra.DETALLES_COMPRAS.Where(Function(f) f.EXISTENCIA.PRODUCTO.IDALTERNO = item.IDALTERNO).FirstOrDefault Is Nothing Then
                                    ...
                                    If producto.CANTIDAD < item.CANTIDAD Then
                                        ...
                                        End If
                                    End If
                                    d = New COMPRA_DEVOLUCION_DETALLE
                                    ...
                                    db.COMPRAS_DEVOLUCIONES_DETALLES.Add(d)
                                    Dim k As New KARDEX
    ...
    Private Sub LoadMov(ByVal v As COMPRA_DEVOLUCION)
        Try
            Me.FormLoad = False
            ...
            For Each d In v.COMPRAS_DEVOLUCIONES_DETALLES
    ...
    Public Sub LoadInfo(Optional ByVal vInt As COMPRA_DEVOLUCION = Nothing, Optional ByVal ByInt As Boolean = False)
        Try
            If txtIdSerie.Text <> "" Then
                If Not ByInt Then
                    Using db As New CodeFirst
                        Dim v = db.COMPRAS_DEVOLUCIONES.Where(Function(f) f.IDSERIE = txtIdSerie.Text And f.CONSECUTIVO = txtCodigo.Text).FirstOrDefault()
        ...
        Try
            If MessageBox.Show("¿Desea anular esta devolución?", "Pregunta de seguridad", System.Windows.Forms.MessageBoxButtons.YesNo, System.Windows.Forms.MessageBoxIcon.Information) = Windows.Forms.DialogResult.Yes Then
                If Not Me.Id.Trim() = "" Then
                    Using db As New CodeFirst
                        Dim v = db.COMPRAS_DEVOLUCIONES.Where(Function(f) f.IDDEVOLUCION = Me.Id And f.ANULADO = "N").FirstOrDefault()
                        If Not v Is Nothing Then
                            If Config.ValidarPeriodo(v.FECHADEVOLUCION) Then
                                v.ANULADO = "S" : db.Entry(v).State = EntityState.Modified

                                For Each item In v.COMPRAS_DEVOLUCIONES_DETALLES
...
#End Region
    ...
    Private Sub btImprimir_Click(sender As Object, e As EventArgs) Handles btImprimir.Click
        If MessageBox.Show("¿Imprimir como ticket?", "Pregunta de seguridad", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
            Try
                Using db As New CodeFirst
                    Dim devolucion = db.COMPRAS_DEVOLUCIONES.Where(Function(f) f.IDDEVOLUCION = Me.Id).FirstOrDefault
                    If Not devolucion Is Nothing Then
                        If devolucion.ANULADO = "N" Then
                            ...
                            If tick.ImpresoraExistente(Config.PrintName) Then
                                ...
                                End If
                                For Each detalle In From pro In db.PRODUCTOS Join exi In db.EXISTENCIAS On pro.IDPRODUCTO Equals exi.IDPRODUCTO Join det In db.COMPRAS_DEVOLUCIONES_DETALLES On exi.IDEXISTENCIA Equals det.IDEXISTENCIA Where det.IDDEVOLUCION = devolucion.IDDEVOLUCION Select pro.IDALTERNO, pro.IDORIGINAL, PRODUCTO = pro.DESCRIPCION, pro.MODELO, pro.UNIDAD_DE_MEDIDA, det.CANTIDAD_DEVUELTA, det.PRECIOUNITARIO_C, det.SUBTOTAL_C
